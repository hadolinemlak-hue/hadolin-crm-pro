...
async function savePortfolio() {
    ...
    try {
        if (state.editingPortfolio) {
            const updated = await api.put('portfolios', state.editingPortfolio, data);
            const idx = state.portfolios.findIndex(p => p.id === state.editingPortfolio);
            if (idx !== -1) state.portfolios[idx] = updated;
            toast('Portföy güncellendi!');
        } else {
            const created = await api.post('portfolios', data);
            state.portfolios.unshift(created);
...
function renderCustomers(filterTur = '', filterDurum = '') {
    ...
    tbody.innerHTML = list.map(c => {
        ...
        <tr>
            ...
            <td style="font-size:.8rem">${c.tercih_ilce || ''} ${c.tercih_il ? ', ' + c.tercih_il : ''}</td>
            <td>
...
async function saveCustomer() {
    ...
    try {
        if (state.editingCustomer) {
            const updated = await api.put('customers', state.editingCustomer, data);
...
function calculateCompatibility(customer, portfolio) {
    ...
    else return 0; // Tür uyuşmuyorsa eşleşme yok

    // Durum uyumu (zorunlu)
    maxScore += 30;
    if (customer.istek_durum === portfolio.durum) score += 30;
    else return 0; // Durum uyuşmuyorsa eşleşme yok
    ...
    if (fiyat >= min && fiyat <= max) {
        score += 20;
    } else if (fiyat <= max * 1.1) {
        score += 10; // %10 aşım tolerans
    }
...
function handleGlobalSearch(query) {
    ...
    if (state.currentPage === 'portfolios') {
        ...
        if (filtered.length === 0) {
            ...
            const tmpPortfolios = state.portfolios;
            state.portfolios = filtered;
            renderPortfolios();
            state.portfolios = tmpPortfolios;
        }
    }
    ...
    if (state.currentPage === 'customers') {
        ...
        const tmpCustomers = state.customers;
        state.customers = filtered;
        renderCustomers();
        state.customers = tmpCustomers;
    }
}
